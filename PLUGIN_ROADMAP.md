...
- [~] **Smoke test the package** — Windows done 2026-07-15 against a simulated fresh environment (extracted bundle + `$CLAUDE_PLUGIN_ROOT` layout, empty `JOB_TAILOR_HOME`, no repo): first-run onboarding, CV setup, offer+cold preflight, CLI (count/company-lists/timeline/record-application/check-duplicate/renames/check-forbidden-labels/probe-url), schema validation, and full document generation (DOCX + PDF + TXT + HTML + run_summary) all pass. Two bundle-only bugs found and fixed in the process (function-level bare imports crashing first-run via `-m`; `~/.claude` mistaken for a project root scattering packs into `~/output`). The real `/plugin install` flow was verified 2026-07-15 (Windows, isolated `CLAUDE_CONFIG_DIR` + `CLAUDE_CODE_PLUGIN_CACHE_DIR`): `.claude-plugin/marketplace.json` added, marketplace add → install job-prospecting@ai-job-prospecting (v1.0.0) → five skills discovered → headless `/job-prospecting:job-status` executed its CLI from the cache and honored `JOB_TAILOR_HOME`. Still open: Mac + Linux (LibreOffice/pandoc PDF fallbacks untested live).
- [x] **Tag v1.0.0 in git**, push. *(Done 2026-07-15 on user confirmation: annotated `v1.0.0` + marketplace-convention `job-prospecting--v1.0.0`, pushed with main.)*
...
- *2026-07-15 — Marketplace plugin entry uses `source: "./"` + `skills: "./.claude/skills/"` instead of a committed built tree: for a root-source plugin the `skills` override replaces the default `skills/` scan, so the repo can be installed as-is and dev/marketplace layouts stay identical. The `scripts/package.py` bundle remains for the `--plugin-dir` / zip paths. Caveat learned in testing: local-path marketplace adds copy the working tree verbatim (`.gitignore` not respected) — never test from a working copy holding personal data.*
